...
## 카카오 로그인 (진우)
- 번거로운 회원가입 없이 빠르고 간편하게 서비스를 이용할 수 있도록 소셜 로그인을 적용했고, 처음엔 구글 로그인으로 기획되었으나 국내에서 서비스하는 사이트이기 때문에 카카오 로그인 서비스가 좀 더 적합하다고 생각되어 카카오 로그인으로 구현하게 되었다. 카카오 로그인 API를 사용했고 REST API 방식으로 구현했다.

## 마이페이지 (진우)
- 현재 구매 또는 판매 했던 이력들, 그리고 잔여 포인트와 프로필을 한눈에 볼 수 있는 마이페이지 구현, 상품을 보다가 마음에 드는 상품을 골라서 담아둘 수 있는 위시리스트 섹션 구현.
 
## 상품리스트페이지 (진우)
- 필요한 상품만 한번에 찾아볼 수 있게하는 검색 기능 구현
