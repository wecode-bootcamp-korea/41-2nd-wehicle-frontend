...
const Category = ({ categoryData, selectOption }) => {
  ...
  const handleFilter = option => {
    const current = searchParams.get(`${option}`);
    if (current === '1') {
      searchParams.delete(`${option}`);
      setSearchParams(searchParams);
      return;
    }
    searchParams.set(`${option}`, '1');
          ...
          categoryList.map(category => {
            return (
              <Hide key={category.id} isFilterTab={isFilterTab}>
                <SelectCategory onClick={selectOption}>
                  <CheckBox
                    onClick={() => handleFilter(category.query)}
                    type="checkBox"
                    checked={searchParams.get(category.query) === '1'}
                    readOnly
                  />
                  {category.name}
                </SelectCategory>
