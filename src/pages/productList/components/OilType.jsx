...
const Category = ({ categoryData, selectOilType }) => {
  ...
  const handleFilter = oilType => {
    const current = searchParams.get('oil');
    if (current === oilType) {
      searchParams.delete('oil');
      setSearchParams(searchParams);
      return;
    }
    searchParams.set('oil', oilType);
          ...
          categoryList.map(category => {
            return (
              <Hide key={category.id} isFilterTab={isFilterTab}>
                <SelectCategory onClick={() => selectOilType(category)}>
                  <CheckBox
                    onClick={() => handleFilter(category.query)}
                    type="checkBox"
                    checked={searchParams.get('oil') === category.query}
                    readOnly
                  />
                  {category.name}
                </SelectCategory>
